...
void perfil(int c,String poses){
  ...
  int casa=casa_h[c]+casa_m[c];  //total casados h y m
  int solte=solte_h[c]+solte_m[c];  //total solteros h y m
  int p_h=sex_h[c]*100;   //porcentaje hombres
  int p_m=sex_m[c]*100;   //porcentaje mujer
  int porcen_h=round(p_h/sex_total[c]);   //porcentaje hombres
  int porcen_m=round(p_m/sex_total[c]);  //porcentaje mujer
